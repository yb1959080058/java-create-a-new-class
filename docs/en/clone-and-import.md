...
`git clone https://github.com/ByteLegendQuest/java-create-a-new-class`

A project directory named `java-create-a-new-class` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-create-a-new-class` directory which is created just now.
